...
CREATE TABLE LOCATION (
        LOCATION_ID             SMALLINT COMMENT 'Central location corporate location code.',
        REGIONAL_GROUP          VARCHAR(20) COMMENT 'Metropolitan center closest to location.');

CREATE TABLE DEPARTMENT (
        DEPARTMENT_ID           TINYINT COMMENT 'Department number: Unique 2-digit department number assigned to each department.',
        NAME                    VARCHAR(14) COMMENT 'Department name: Only the first 14 characters of a department name can be stored in the database.  Storing names in all caps is recommended but not required.',
        LOCATION_ID             SMALLINT COMMENT 'Department location: The code for where the department''s central office is located.  (LOCATION)');

CREATE TABLE JOB (
        JOB_ID                  SMALLINT COMMENT 'Unique 3-digit code for job function and title.',
        FUNCTION                VARCHAR(30) COMMENT 'Generic function class of job.');

CREATE TABLE EMPLOYEE (
        EMPLOYEE_ID             SMALLINT COMMENT 'Employee Number: Unique 4-digit identification number assigned to every employee.',
        LAST_NAME               VARCHAR(15) COMMENT 'Last name of employee.  Stored in all caps.',
        FIRST_NAME              VARCHAR(15) COMMENT 'First name of employee.  Stored in all caps.',
        MIDDLE_INITIAL          VARCHAR(1) COMMENT 'Middle initial (one character only).  Stored in all caps.',
        JOB_ID                  SMALLINT COMMENT 'Job identification number.  See JOB for descriptions.',
        MANAGER_ID              SMALLINT COMMENT 'Manager: The employee number of this employee''s manager.  If employee has no manager, value should be NULL.',
        HIRE_DATE               DATETIME COMMENT 'Date the employee was HIRED, not necessarily the day the employee STARTED.',
        SALARY                  DECIMAL(7,2) COMMENT 'Monthly salary for the employee in U.S. dollars.  Salary grade information can be found in the table SALARY_GRADE.',
        COMMISSION              DECIMAL(7,2) COMMENT 'Commission (in U.S. dollars) earned last month.  This field should be NULL for non-commissioned employees.',
        DEPARTMENT_ID           TINYINT COMMENT 'Employee''s department number.  Further department information is available in the table DEPT.');

CREATE TABLE SALARY_GRADE (
        GRADE_ID                SMALLINT COMMENT 'Number used to grade salaries.  Higher salary grade numbers denote higher salaries.',
        LOWER_BOUND             DECIMAL(7,2) COMMENT 'The lowest monthly salary (rounded to the nearest U.S. dollar) that applies to a particular salary grade. (i.e. the condition ROUND(EMP.SAL) >= LOWER_BOUND must be met.)',
        UPPER_BOUND             DECIMAL(7,2) COMMENT 'The highest monthly salary (rounded to the nearest U.S. dollar) that applies to a particular salary grade. (i.e. the condition ROUND(EMP.SAL) <= UPPER_BOUND must be met.)');

CREATE TABLE PRODUCT (
        PRODUCT_ID              INT COMMENT 'Unique 6 digit number assigned to all products.  Number generated by sequence PRODUCT_ID.',
        DESCRIPTION             VARCHAR (30) COMMENT 'Full product name (up to 30 characters).  Storing product names in all caps is recommended but not required.');
 
CREATE TABLE PRICE (
        PRODUCT_ID              INT COMMENT 'Product number to which price applies.  Product name found in table PRICE.',
        LIST_PRICE              DECIMAL (8,2) COMMENT 'Undiscounted price (in U.S. dollars).',
        MIN_PRICE               DECIMAL (8,2) COMMENT 'Lowest price (in U.S. dollars) that can be offered.',
        START_DATE              DATETIME COMMENT 'Date which standard and minimum prices go into effect.',
        END_DATE                DATETIME COMMENT 'Date which standard and minimum price expire.  This value can be left NULL.');

CREATE TABLE CUSTOMER (
        CUSTOMER_ID             INT COMMENT 'Unique 6 digit number assigned to all customers.  Number generated by the sequence CUSTOMER_ID.',
        NAME                    VARCHAR (45) COMMENT 'Full store name of every customer (up to 45 characters).  Storing names in all caps is recommended but not required.',
        ADDRESS                 VARCHAR (40) COMMENT 'Street address of customer.  CITY, STATE, and ZIP_CODE make up the rest of the address.',
        CITY                    VARCHAR (30) COMMENT 'City name for address of customer.  Storing city names in all caps is recommended, but not required.',
        STATE                   VARCHAR (2) COMMENT 'Two letter abbreviation for state name for address of customer.  Must be entered iùý}ÿÿÿÿ÷ÿþÿÏ}÷ßÿýï?÷ÿþÿwÿó}÷ÿïÿÛÿwmmûûoÿoü¾wÿ|ÿþo÷oû÷ÿÿÿuw~ÿýw÷wöÿ~wûçoô{srmgxtëwmwpiwlfes.#³Ïýwkïs"skowmmìgo+omwuìsæsoou;,w{wm}lÿ}yw.w¿íOçÿÿOíß_ý}Gÿl_ÿÿïõÛ÷ïoþ}wå÷ÿçÿí}msíïïwýÿ÷÷ïÿï}þÿo}ÿÿ{÷{ïÿÿÿïÿ~oï¿ÿÏ_ÿm÷÷}ïÿeÏßÿÿw_ÿßÿïßöïß{ÿßußÿÿ_ÿÿïûçýÿo}mÿïÿúÿn²÷ÿ~ÿýïÿÿ}ÿ´ÿýÿî~ÿwaôÿý{þowý÷§~gv}mú÷uý~emwsçã}kfoämþíî}sô}÷fowí}óýÿhdmgoþwëooy9~¯ûMÛGïMÏUÿtàoîuûÿìUmnég÷÷tOmoö~ûlÿûýöÿnùnÿ÷yÿuõÿÿ÷/ÿÿÿÿÿÿï}ÿ~wþÿÿýÿÿnoÿÿ}ýoþooðwÿuÿr''s account.  Employee information can be found in the EMPLOYEE table.',
        ZIP_CODE                VARCHAR (9),
        AREA_CODE               SMALLINT,
        PHONE_NUMBER            INT,
        SALESPERSON_ID          SMALLINT,
        CREDIT_LIMIT            DECIMAL (9,2) COMMENT 'Credit limit of customer or prospective customer in U.S. dollars.',
        COMMENTS                LONGTEXT COMMENT 'Field can be used flexibly.  Suggestions for use include lists of action items or special considerations when dealing with the particular customer.');

CREATE TABLE SALES_ORDER  (
        ORDER_ID                SMALLINT COMMENT 'Unique 4 digit number assigned to all orders.  Number generated by sequence ORDER_ID.',
        ORDER_DATE              DATETIME COMMENT 'Date sales order logged (not necessarily when order was fulfilled.)',
        CUSTOMER_ID             INT COMMENT 'Customer ordering the goods.  Further information can be found in the table CUSTOMER.',
        SHIP_DATE               DATETIME COMMENT 'Date order fulfilled and invoice sent (not necessarily date payment received).',
        TOTAL                   DECIMAL (8,2) COMMENT 'Total of all items totals in the order.  (i.e. the value of TOTAL for order N should contain: SELECT SUM(TOTAL) FROM ITEM WHERE ORDER_ID=N)');

CREATE TABLE ITEM  (
        ORDER_ID                SMALLINT COMMENT 'Sales order in which this item appears.  Sales order information can be found in table SALES_ORDER.',
        ITEM_ID                 SMALLINT COMMENT 'Number of item within the sales order.',
        PRODUCT_ID              INT COMMENT 'Number of product (described in PRODUCT) that pertains to the line item.',
        ACTUAL_PRICE            DECIMAL (8,2) COMMENT 'Price to be paid by the customer in U.S. dollars.  The standard price is PRICE.STANDARD_PRICE, but in some instances, discounts are given to as low as PRICE.MINIMUM_PRICE.',
        QUANTITY                INT COMMENT 'The amount of product to be ordered in this line item.',
        TOTAL                   DECIMAL (8,2) COMMENT 'The total price (in U.S. dollars) of this line item.  TOTAL = ACTUAL_PRICE * QUANTITY.');
...
INSERT INTO EMPLOYEE VALUES
    (7369,'SMITH','JOHN','Q',667,7902,STR_TO_DATE(2446052,'J'),800,NULL,20);
INSERT INTO EMPLOYEE VALUES
    (7499,'ALLEN','KEVIN','J',670,7698,STR_TO_DATE(2446117,'J'),1600,300,30);
INSERT INTO EMPLOYEE VALUES
    (7505,'DOYLE','JEAN','K',671,7839,STR_TO_DATE(2446160,'J'),2850,NULL,13);
INSERT INTO EMPLOYEE VALUES
    (7506,'DENNIS','LYNN','S',671,7839,STR_TO_DATE(2446201,'J'),2750,NULL,23);
INSERT INTO EMPLOYEE VALUES
    (7507,'BAKER','LESLIE','D',671,7839,STR_TO_DATE(2446227,'J'),2200,NULL,14);
INSERT INTO EMPLOYEE VALUES
    (7521,'WARD','CYNTHIA','D',670,7698,STR_TO_DATE(2446119,'J'),1250,500,30);
INSERT INTO EMPLOYEE VALUES
    (7555,'PETERS','DANIEL','T',670,7505,STR_TO_DATE(2446156,'J'),1250,300,13);
INSERT INTO EMPLOYEE VALUES
    (7557,'SHAW','KAREN','P',670,7505,STR_TO_DATE(2446158,'J'),1250,1200,13);
INSERT INTO EMPLOYEE VALUES
    (7560,'DUNCAN','SARAH','S',670,7506,STR_TO_DATE(2446217,'J'),1250,NULL,23);
INSERT INTO EMPLOYEE VALUES
    (7564,'LANGE','GREGORY','J',670,7506,STR_TO_DATE(2446218,'J'),1250,300,23);
INSERT INTO EMPLOYEE VALUES
    (7566,'JONES','TERRY','M',671,7839,STR_TO_DATE(2446158,'J'),2975,NULL,20);
INSERT INTO EMPLOYEE VALUES
    (7569,'ALBERTS','CHRIS','L',671,7839,STR_TO_DATE(2446162,'J'),3000,NULL,12);
INSERT INTO EMPLOYEE VALUES
    (7600,'PORTER','RAYMOND','Y',670,7505,STR_TO_DATE(2446171,'J'),1250,900,13);
INSERT INTO EMPLOYEE VALUES
    (7609,'LEWIS','RICHARD','M',668,7507,STR_TO_DATE(2446172,'J'),1800,NULL,24);
INSERT INTO EMPLOYEE VALUES
    (7654,'MARTIN','KENNETH','J',670,7698,STR_TO_DATE(2446337,'J'),1250,1400,30);
INSERT INTO EMPLOYEE VALUES
    (7676,'SOMMERS','DENISE','D',668,7507,STR_TO_DATE(2446175,'J'),1850,NULL,34);
INSERT INTO EMPLOYEE VALUES
    (7698,'BLAKE','MARION','S',671,7839,STR_TO_DATE(2446187,'J'),2850,NULL,30);
INSERT INTO EMPLOYEE VALUES
    (7782,'CLARK','CAROL','F',671,7839,STR_TO_DATE(2446226,'J'),2450,NULL,10);
INSERT INTO EMPLOYEE VALUES
    (7788,'SCOTT','DONALD','T',669,7566,STR_TO_DATE(2446774,'J'),3000,NULL,20);
INSERT INTO EMPLOYEE VALUES
    (7789,'WEST','LIVIA','N',670,7506,STR_TO_DATE(2446160,'J'),1500,1000,23);
INSERT INTO EMPLOYEE VALUES
    (7799,'FISHER','MATTHEW','G',669,7569,STR_TO_DATE(2446777,'J'),3000,NULL,12);
INSERT INTO EMPLOYEE VALUES
    (7820,'ROSS','PAUL','S',670,7505,STR_TO_DATE(2446218,'J'),1300,800,43);
INSERT INTO EMPLOYEE VALUES
    (7839,'KING','FRANCIS','A',672,NULL,STR_TO_DATE(2446387,'J'),5000,NULL,10);
INSERT INTO EMPLOYEE VALUES
    (7844,'TURNER','MARY','A',670,7698,STR_TO_DATE(2446317,'J'),1500,0,30);
INSERT INTO EMPLOYEE VALUES
    (7876,'ADAMS','DIANE','G',667,7788,STR_TO_DATE(2446808,'J'),1100,NULL,20);
INSERT INTO EMPLOYEE VALUES
    (7900,'JAMES','FRED','S',667,7698,STR_TO_DATE(2446403,'J'),950,NULL,30);
INSERT INTO EMPLOYEE VALUES
    (7902,'FORD','JENNIFER','D',669,7566,STR_TO_DATE(2446403,'J'),3000,NULL,20);
INSERT INTO EMPLOYEE VALUES
    (7916,'ROBERTS','GRACE','M',669,7569,STR_TO_DATE(2446800,'J'),2875,NULL,12);
INSERT INTO EMPLOYEE VALUES
    (7919,'DOUGLAS','MICHAEL','A',667,7799,STR_TO_DATE(2446800,'J'),800,NULL,12);
INSERT INTO EMPLOYEE VALUES
    (7934,'MILLER','BARBARA','M',667,7782,STR_TO_DATE(2446454,'J'),1300,NULL,10);
INSERT INTO EMPLOYEE VALUES
    (7950,'JENSEN','ALICE','B',667,7505,STR_TO_DATE(2446811,'J'),750,NULL,13);
INSERT INTO EMPLOYEE VALUES
    (7954,'MURRAY','JAMES','T',667,7506,STR_TO_DATE(2446812,'J'),750,NULL,23);

INSERT INTO PRICE VALUES
   (100871,4.8,3.2,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447862,'J'));
INSERT INTO PRICE VALUES
   (100890,58,46.4,STR_TO_DATE(2447528,'J'),NULL);
INSERT INTO PRICE VALUES
   (100890,54,40.5,STR_TO_DATE(2447314,'J'),STR_TO_DATE(2447527,'J'));
INSERT INTO PRICE VALUES
   (100860,35,28,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (100860,32,25.6,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (100860,30,24,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (100861,45,36,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (100861,42,33.6,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (100861,39,31.2,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (100870,2.8,2.4,STR_TO_DATE(2447893,'J'),NULL);
INSERT INTO PRICE VALUES
   (100870,2.4,1.9,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447862,'J'));
INSERT INTO PRICE VALUES
   (100871,5.6,4.8,STR_TO_DATE(2447893,'J'),NULL);
INSERT INTO PRICE VALUES
   (101860,24,18,STR_TO_DATE(2447573,'J'),NULL);
INSERT INTO PRICE VALUES
   (101863,12.5,9.4,STR_TO_DATE(2447573,'J'),NULL);
INSERT INTO PRICE VALUES
   (102130,3.4,2.8,STR_TO_DATE(2447757,'J'),NULL);
INSERT INTO PRICE VALUES
   (200376,2.4,1.75,STR_TO_DATE(2448211,'J'),NULL);
INSERT INTO PRICE VALUES
   (200380,4,3.2,STR_TO_DATE(2448211,'J'),NULL);
INSERT INTO PRICE VALUES
   (103120,23.2,18.6,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (103120,24,19.2,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (103120,25,20,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (103121,27.8,22.3,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (103121,28.8,23,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (103121,30,24,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (103130,4,3.5,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (103130,4.2,3.6,STR_TO_DATE(2447893,'J'),NULL);
INSERT INTO PRICE VALUES
   (103131,4.2,3.6,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (103131,4.5,3.9,STR_TO_DATE(2447893,'J'),NULL);
INSERT INTO PRICE VALUES
   (103140,20,15,STR_TO_DATE(2447573,'J'),NULL);
INSERT INTO PRICE VALUES
   (103141,20,15,STR_TO_DATE(2447573,'J'),NULL);
INSERT INTO PRICE VALUES
   (102132,3.4,2.8,STR_TO_DATE(2447757,'J'),NULL);
INSERT INTO PRICE VALUES
   (104350,40,32,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (104350,42,33.6,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (104350,44,35.3,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (104351,23.6,18.9,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (104351,24.8,19.8,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (104351,26,20.8,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (104352,50,35,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (104352,54,37.8,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (104352,58.3,40.8,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (104360,36,29.5,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (104360,39,32,STR_TO_DATE(2447893,'J'),NULL);
INSERT INTO PRICE VALUES
   (104361,47,33,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (104361,50,35,STR_TO_DATE(2447893,'J'),NULL);
INSERT INTO PRICE VALUES
   (104362,5,4,STR_TO_DATE(2447573,'J'),NULL);
INSERT INTO PRICE VALUES
   (102134,3.4,2.8,STR_TO_DATE(2447757,'J'),NULL);
INSERT INTO PRICE VALUES
   (105123,36.3,29,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (105123,37.7,32,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (105123,40,34,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (105124,10,7.6,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (105124,12,9,STR_TO_DATE(2447893,'J'),STR_TO_DATE(2448043,'J'));
INSERT INTO PRICE VALUES
   (105124,15,10,STR_TO_DATE(2448044,'J'),NULL);
INSERT INTO PRICE VALUES
   (105125,2,1.7,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (105125,3,2.6,STR_TO_DATE(2447893,'J'),NULL);
INSERT INTO PRICE VALUES
   (105126,5,4.3,STR_TO_DATE(2447528,'J'),STR_TO_DATE(2447892,'J'));
INSERT INTO PRICE VALUES
   (105126,6,5,STR_TO_DATE(2447893,'J'),NULL);
INSERT INTO PRICE VALUES
   (105127,6,5,STR_TO_DATE(2448211,'J'),NULL);
INSERT INTO PRICE VALUES
   (105128,10,8,STR_TO_DATE(2448211,'J'),NULL);
INSERT INTO PRICE VALUES
   (102136,3.4,2.8,STR_TO_DATE(2447757,'J'),NULL);
...
INSERT INTO SALES_ORDER VALUES
   (610,STR_TO_DATE(2448264,'J'),101,STR_TO_DATE(2448265,'J'),101.4);
INSERT INTO SALES_ORDER VALUES
   (611,STR_TO_DATE(2448268,'J'),102,STR_TO_DATE(2448268,'J'),45);
INSERT INTO SALES_ORDER VALUES
   (612,STR_TO_DATE(2448272,'J'),104,STR_TO_DATE(2448277,'J'),5860);
INSERT INTO SALES_ORDER VALUES
   (601,STR_TO_DATE(2448013,'J'),106,STR_TO_DATE(2448042,'J'),60.8);
INSERT INTO SALES_ORDER VALUES
   (602,STR_TO_DATE(2448048,'J'),102,STR_TO_DATE(2448063,'J'),56);
INSERT INTO SALES_ORDER VALUES
   (600,STR_TO_DATE(2448013,'J'),103,STR_TO_DATE(2448041,'J'),42);
INSERT INTO SALES_ORDER VALUES
   (604,STR_TO_DATE(2448058,'J'),106,STR_TO_DATE(2448073,'J'),642);
INSERT INTO SALES_ORDER VALUES
   (605,STR_TO_DATE(2448087,'J'),106,STR_TO_DATE(2448103,'J'),8374);
INSERT INTO SALES_ORDER VALUES
   (606,STR_TO_DATE(2448087,'J'),100,STR_TO_DATE(2448103,'J'),3.4);
INSERT INTO SALES_ORDER VALUES
   (609,STR_TO_DATE(2448105,'J'),100,STR_TO_DATE(2448119,'J'),102.5);
INSERT INTO SALES_ORDER VALUES
   (607,STR_TO_DATE(2448091,'J'),104,STR_TO_DATE(2448091,'J'),5.6);
INSERT INTO SALES_ORDER VALUES
   (608,STR_TO_DATE(2448098,'J'),104,STR_TO_DATE(2448098,'J'),35.2);
INSERT INTO SALES_ORDER VALUES
   (603,STR_TO_DATE(2448048,'J'),102,STR_TO_DATE(2448048,'J'),224);
INSERT INTO SALES_ORDER VALUES
   (620,STR_TO_DATE(2448328,'J'),100,STR_TO_DATE(2448328,'J'),4450);
INSERT INTO SALES_ORDER VALUES
   (613,STR_TO_DATE(2448289,'J'),108,STR_TO_DATE(2448289,'J'),6400);
INSERT INTO SALES_ORDER VALUES
   (614,STR_TO_DATE(2448289,'J'),102,STR_TO_DATE(2448293,'J'),23940);
INSERT INTO SALES_ORDER VALUES
   (616,STR_TO_DATE(2448291,'J'),103,STR_TO_DATE(2448298,'J'),764);
INSERT INTO SALES_ORDER VALUES
   (619,STR_TO_DATE(2448310,'J'),104,STR_TO_DATE(2448320,'J'),1260);
INSERT INTO SALES_ORDER VALUES
   (617,STR_TO_DATE(2448293,'J'),105,STR_TO_DATE(2448319,'J'),46370);
INSERT INTO SALES_ORDER VALUES
   (615,STR_TO_DATE(2448289,'J'),107,STR_TO_DATE(2448294,'J'),710);
INSERT INTO SALES_ORDER VALUES
   (618,STR_TO_DATE(2448303,'J'),102,STR_TO_DATE(2448322,'J'),3083);
INSERT INTO SALES_ORDER VALUES
   (621,STR_TO_DATE(2448331,'J'),100,STR_TO_DATE(2448348,'J'),730);
INSERT INTO SALES_ORDER VALUES
   (509,STR_TO_DATE(2447682,'J'),226,STR_TO_DATE(2447688,'J'),1174);
INSERT INTO SALES_ORDER VALUES
   (523,STR_TO_DATE(2447936,'J'),226,STR_TO_DATE(2447936,'J'),1165);
INSERT INTO SALES_ORDER VALUES
   (549,STR_TO_DATE(2448253,'J'),226,STR_TO_DATE(2448271,'J'),1620);
INSERT INTO SALES_ORDER VALUES
   (507,STR_TO_DATE(2447673,'J'),228,STR_TO_DATE(2447674,'J'),886);
INSERT INTO SALES_ORDER VALUES
   (516,STR_TO_DATE(2447800,'J'),228,STR_TO_DATE(2447803,'J'),1815);
INSERT INTO SALES_ORDER VALUES
   (553,STR_TO_DATE(2448290,'J'),228,STR_TO_DATE(2448301,'J'),4400);
INSERT INTO SALES_ORDER VALUES
   (526,STR_TO_DATE(2447955,'J'),221,STR_TO_DATE(2447958,'J'),7700);
INSERT INTO SALES_ORDER VALUES
   (543,STR_TO_DATE(2448139,'J'),221,STR_TO_DATE(2448142,'J'),8400);
INSERT INTO SALES_ORDER VALUES
   (555,STR_TO_DATE(2448320,'J'),221,STR_TO_DATE(2448323,'J'),8540);
INSERT INTO SALES_ORDER VALUES
   (559,STR_TO_DATE(2448212,'J'),222,STR_TO_DATE(2448370,'J'),387.2);
INSERT INTO SALES_ORDER VALUES
   (528,STR_TO_DATE(2447975,'J'),224,STR_TO_DATE(2447975,'J'),3770);
INSERT INTO SALES_ORDER VALUES
   (531,STR_TO_DATE(2447982,'J'),224,STR_TO_DATE(2447984,'J'),1400);
INSERT INTO SALES_ORDER VALUES
   (558,STR_TO_DATE(2448347,'J'),224,STR_TO_DATE(2448349,'J'),1700);
INSERT INTO SALES_ORDER VALUES
   (525,STR_TO_DATE(2447951,'J'),225,STR_TO_DATE(2447954,'J'),377);
INSERT INTO SALES_ORDER VALUES
   (552,STR_TO_DATE(2448290,'J'),223,STR_TO_DATE(2448295,'J'),555.8);
INSERT INTO SALES_ORDER VALUES
   (556,STR_TO_DATE(2448326,'J'),223,STR_TO_DATE(2448328,'J'),85);
INSERT INTO SALES_ORDER VALUES
   (560,STR_TO_DATE(2448372,'J'),223,STR_TO_DATE(2448372,'J'),72);
INSERT INTO SALES_ORDER VALUES
   (565,STR_TO_DATE(2448409,'J'),227,STR_TO_DATE(2448411,'J'),4900);
INSERT INTO SALES_ORDER VALUES
   (574,STR_TO_DATE(2448540,'J'),201,STR_TO_DATE(2448542,'J'),1685);
INSERT INTO SALES_ORDER VALUES
   (576,STR_TO_DATE(2448543,'J'),201,STR_TO_DATE(2448548,'J'),2058.9);
INSERT INTO SALES_ORDER VALUES
   (503,STR_TO_DATE(2447611,'J'),201,STR_TO_DATE(2447623,'J'),1876);
INSERT INTO SALES_ORDER VALUES
   (518,STR_TO_DATE(2447820,'J'),201,STR_TO_DATE(2447836,'J'),2932.5);
INSERT INTO SALES_ORDER VALUES
   (517,STR_TO_DATE(2447823,'J'),201,STR_TO_DATE(2447838,'J'),784);
INSERT INTO SALES_ORDER VALUES
   (544,STR_TO_DATE(2448146,'J'),202,STR_TO_DATE(2448150,'J'),2358);
INSERT INTO SALES_ORDER VALUES
   (524,STR_TO_DATE(2447945,'J'),202,STR_TO_DATE(2447955,'J'),1979);
INSERT INTO SALES_ORDER VALUES
   (502,STR_TO_DATE(2447568,'J'),202,STR_TO_DATE(2447581,'J'),500);
INSERT INTO SALES_ORDER VALUES
   (539,STR_TO_DATE(2448069,'J'),202,STR_TO_DATE(2448076,'J'),1300);
INSERT INTO SALES_ORDER VALUES
   (511,STR_TO_DATE(2447756,'J'),202,STR_TO_DATE(2447760,'J'),647);
INSERT INTO SALES_ORDER VALUES
   (512,STR_TO_DATE(2447767,'J'),203,STR_TO_DATE(2447783,'J'),428);
INSERT INTO SALES_ORDER VALUES
   (562,STR_TO_DATE(2448381,'J'),203,STR_TO_DATE(2448396,'J'),2044.5);
INSERT INTO SALES_ORDER VALUES
   (529,STR_TO_DATE(2447984,'J'),203,STR_TO_DATE(2447987,'J'),1264.7);
INSERT INTO SALES_ORDER VALUES
   (563,STR_TO_DATE(2448402,'J'),204,STR_TO_DATE(2448403,'J'),889);
INSERT INTO SALES_ORDER VALUES
   (568,STR_TO_DATE(2448451,'J'),204,STR_TO_DATE(2448459,'J'),1217.4);
INSERT INTO SALES_ORDER VALUES
   (504,STR_TO_DATE(2447628,'J'),204,STR_TO_DATE(2447629,'J'),1434.7);
INSERT INTO SALES_ORDER VALUES
   (538,STR_TO_DATE(2448066,'J'),204,STR_TO_DATE(2448069,'J'),741);
INSERT INTO SALES_ORDER VALUES
   (535,STR_TO_DATE(2448040,'J'),204,STR_TO_DATE(2448052,'J'),810);
INSERT INTO SALES_ORDER VALUES
   (578,STR_TO_DATE(2448580,'J'),204,STR_TO_DATE(2448595,'J'),2275.6);
INSERT INTO SALES_ORDER VALUES
   (534,STR_TO_DATE(2448023,'J'),206,STR_TO_DATE(2448027,'J'),420);
INSERT INTO SALES_ORDER VALUES
   (536,STR_TO_DATE(2448033,'J'),206,STR_TO_DATE(2448049,'J'),2135.6);
INSERT INTO SALES_ORDER VALUES
   (572,STR_TO_DATE(2448488,'J'),206,STR_TO_DATE(2448493,'J'),1200.5);
INSERT INTO SALES_ORDER VALUES
   (514,STR_TO_DATE(2447775,'J'),207,STR_TO_DATE(2447786,'J'),1140);
INSERT INTO SALES_ORDER VALUES
   (521,STR_TO_DATE(2447851,'J'),207,STR_TO_DATE(2447854,'J'),896.9);
INSERT INTO SALES_ORDER VALUES
   (551,STR_TO_DATE(2448279,'J'),208,STR_TO_DATE(2448294,'J'),1142);
INSERT INTO SALES_ORDER VALUES
   (513,STR_TO_DATE(2447776,'J'),208,STR_TO_DATE(2447784,'J'),1497);
INSERT INTO SALES_ORDER VALUES
   (508,STR_TO_DATE(2447673,'J'),208,STR_TO_DATE(2447686,'J'),1080);
INSERT INTO SALES_ORDER VALUES
   (515,STR_TO_DATE(2447789,'J'),208,STR_TO_DATE(2447796,'J'),1428.5);
INSERT INTO SALES_ORDER VALUES
   (542,STR_TO_DATE(2448115,'J'),208,STR_TO_DATE(2448127,'J'),2409);
INSERT INTO SALES_ORDER VALUES
   (573,STR_TO_DATE(2448540,'J'),201,STR_TO_DATE(2448542,'J'),1627);
INSERT INTO SALES_ORDER VALUES
   (566,STR_TO_DATE(2448417,'J'),201,STR_TO_DATE(2448420,'J'),3389.2);
INSERT INTO SALES_ORDER VALUES
   (520,STR_TO_DATE(2447847,'J'),201,STR_TO_DATE(2447850,'J'),341.6);
INSERT INTO SALES_ORDER VALUES
   (519,STR_TO_DATE(2447820,'J'),201,STR_TO_DATE(2447836,'J'),955);
INSERT INTO SALES_ORDER VALUES
   (575,STR_TO_DATE(2448534,'J'),201,STR_TO_DATE(2448547,'J'),684);
INSERT INTO SALES_ORDER VALUES
   (547,STR_TO_DATE(2448181,'J'),202,STR_TO_DATE(2448192,'J'),984.4);
INSERT INTO SALES_ORDER VALUES
   (540,STR_TO_DATE(2448088,'J'),202,STR_TO_DATE(2448090,'J'),861.2);
INSERT INTO SALES_ORDER VALUES
   (567,STR_TO_DATE(2448443,'J'),202,STR_TO_DATE(2448445,'J'),200);
INSERT INTO SALES_ORDER VALUES
   (570,STR_TO_DATE(2448455,'J'),202,STR_TO_DATE(2448462,'J'),663.6);
INSERT INTO SALES_ORDER VALUES
   (571,STR_TO_DATE(2448471,'J'),202,STR_TO_DATE(2448481,'J'),1095.6);
INSERT INTO SALES_ORDER VALUES
   (541,STR_TO_DATE(2448112,'J'),203,STR_TO_DATE(2448122,'J'),400);
INSERT INTO SALES_ORDER VALUES
   (532,STR_TO_DATE(2447990,'J'),203,STR_TO_DATE(2448003,'J'),1295.4);
INSERT INTO SALES_ORDER VALUES
   (527,STR_TO_DATE(2447950,'J'),204,STR_TO_DATE(2447965,'J'),3054.4);
INSERT INTO SALES_ORDER VALUES
   (501,STR_TO_DATE(2447533,'J'),204,STR_TO_DATE(2447538,'J'),216);
INSERT INTO SALES_ORDER VALUES
   (564,STR_TO_DATE(2448402,'J'),204,STR_TO_DATE(2448403,'J'),898.8);
INSERT INTO SALES_ORDER VALUES
   (537,STR_TO_DATE(2448057,'J'),204,STR_TO_DATE(2448069,'J'),672.9);
INSERT INTO SALES_ORDER VALUES
   (522,STR_TO_DATE(2447899,'J'),204,STR_TO_DATE(2447902,'J'),2578.8);
INSERT INTO SALES_ORDER VALUES
   (554,STR_TO_DATE(2448306,'J'),204,STR_TO_DATE(2448310,'J'),282.8);
INSERT INTO SALES_ORDER VALUES
   (569,STR_TO_DATE(2448454,'J'),205,STR_TO_DATE(2448461,'J'),2073);
INSERT INTO SALES_ORDER VALUES
   (550,STR_TO_DATE(2448284,'J'),205,STR_TO_DATE(2448295,'J'),2356);
INSERT INTO SALES_ORDER VALUES
   (548,STR_TO_DATE(2448222,'J'),205,STR_TO_DATE(2448238,'J'),666.9);
INSERT INTO SALES_ORDER VALUES
   (505,STR_TO_DATE(2447624,'J'),206,STR_TO_DATE(2447639,'J'),650);
INSERT INTO SALES_ORDER VALUES
   (577,STR_TO_DATE(2448536,'J'),206,STR_TO_DATE(2448550,'J'),1265.75);
INSERT INTO SALES_ORDER VALUES
   (533,STR_TO_DATE(2448010,'J'),206,STR_TO_DATE(2448014,'J'),1122.1);
INSERT INTO SALES_ORDER VALUES
   (561,STR_TO_DATE(2448367,'J'),207,STR_TO_DATE(2448384,'J'),2558.3);
INSERT INTO SALES_ORDER VALUES
   (506,STR_TO_DATE(2447644,'J'),208,STR_TO_DATE(2447657,'J'),2600.4);
INSERT INTO SALES_ORDER VALUES
   (530,STR_TO_DATE(2447985,'J'),208,STR_TO_DATE(2447989,'J'),3026.5);
INSERT INTO SALES_ORDER VALUES
   (545,STR_TO_DATE(2448171,'J'),208,STR_TO_DATE(2448178,'J'),475);
INSERT INTO SALES_ORDER VALUES
   (557,STR_TO_DATE(2448324,'J'),208,STR_TO_DATE(2448328,'J'),2461.8);
INSERT INTO SALES_ORDER VALUES
   (546,STR_TO_DATE(2448182,'J'),208,STR_TO_DATE(2448188,'J'),3663);
INSERT INTO SALES_ORDER VALUES
   (510,STR_TO_DATE(2447726,'J'),208,STR_TO_DATE(2447729,'J'),1336.6);
...
ALTER TABLE CUSTOMER ADD /*  ER_ID_NN */
   CHECK (CUSTOMER_ID IS NOT NULL);
ALTER TABLE CUSTOMER ADD /*  ERSON_ID_NN */
   CHECK (SALESPERSON_ID IS NOT NULL);
ALTER TABLE CUSTOMER ADD /*  ER_ID_ZERO */
   CHECK (CUSTOMER_ID > 0);
ALTER TABLE CUSTOMER ADD /*  ALL_CAPS */
   CHECK (STATE = UPPER(STATE));
ALTER TABLE CUSTOMER ADD /*  DE_FULL */
   CHECK (CHAR_LENGTH(IFNULL(ZIP_CODE, '99999')) IN (5, 9));
ALTER TABLE DEPARTMENT ADD /*  _NN */
   CHECK (DEPARTMENT_ID IS NOT NULL);
ALTER TABLE EMPLOYEE ADD /*  NN */
   CHECK (EMPLOYEE_ID IS NOT NULL);
ALTER TABLE EMPLOYEE ADD /*  EE$DEPTNO_NN */
   CHECK (DEPARTMENT_ID IS NOT NULL);
ALTER TABLE ITEM ADD /*  RDER_ID_NN */
   CHECK (ORDER_ID IS NOT NULL);
ALTER TABLE ITEM ADD /*  D_NN */
   CHECK (ITEM_ID IS NOT NULL);
ALTER TABLE JOB ADD /*  NN */
   CHECK (JOB_ID IS NOT NULL);
ALTER TABLE JOB ADD /*  NN */
   CHECK (FUNCTION IS NOT NULL);
ALTER TABLE LOCATION ADD /*   */
   CHECK (LOCATION_ID IS NOT NULL);
ALTER TABLE PRICE ADD /*  PRODUCT_ID_NN */
   CHECK (PRODUCT_ID IS NOT NULL);
ALTER TABLE PRICE ADD /*  ICE_LESS */
   CHECK (LIST_PRICE IS NULL OR MIN_PRICE IS NULL OR MIN_PRICE <= LIST_PRICE);
ALTER TABLE PRICE ADD /*  DATE_LESS */
   CHECK (END_DATE IS NULL OR START_DATE <= END_DATE);
ALTER TABLE PRODUCT ADD /*  T_ID_NN */
   CHECK (PRODUCT_ID IS NOT NULL);
ALTER TABLE SALARY_GRADE ADD /*  ID_NN */
   CHECK (GRADE_ID IS NOT NULL);
ALTER TABLE SALARY_GRADE ADD /*  BOUND_LESS */
...
ALTER TABLE SALES_ORDER ADD /*  ID_NN */
   CHECK (ORDER_ID IS NOT NULL);
ALTER TABLE SALES_ORDER ADD /*  CUSTOMER_ID_NN */
   CHECK (CUSTOMER_ID IS NOT NULL);
ALTER TABLE SALES_ORDER ADD /*  TOTAL_ZERO */
   CHECK (TOTAL >= 0);

ALTER TABLE CUSTOMER ADD /*  ER_PRIMARY_KEY */
   PRIMARY KEY (CUSTOMER_ID);
ALTER TABLE DEPARTMENT ADD /*  RIMARY_KEY */
   PRIMARY KEY (DEPARTMENT_ID);
ALTER TABLE EMPLOYEE ADD /*  IMARY_KEY */
   PRIMARY KEY (EMPLOYEE_ID);
ALTER TABLE ITEM ADD /*  RIMARY_KEY */
   PRIMARY KEY (ORDER_ID, ITEM_ID);
ALTER TABLE JOB ADD /*  IMARY_KEY */
   PRIMARY KEY (JOB_ID);
ALTER TABLE LOCATION ADD /*  IMARY_KEY */
   PRIMARY KEY (LOCATION_ID);
ALTER TABLE PRODUCT ADD /*  T_PRIMARY_KEY */
   PRIMARY KEY (PRODUCT_ID);
ALTER TABLE SALARY_GRADE ADD /*  _GRADE_PRIMARY_KEY */
   PRIMARY KEY (GRADE_ID);
ALTER TABLE SALES_ORDER ADD /*  ORDER_PRIMARY_KEY */
   PRIMARY KEY (ORDER_ID);

ALTER TABLE CUSTOMER ADD /*  ER_FOREIGN_KEY  */
   FOREIGN KEY (SALESPERSON_ID) REFERENCES EMPLOYEE(EMPLOYEE_ID);
ALTER TABLE DEPARTMENT ADD /*  OREIGN_KEY  */
   FOREIGN KEY (LOCATION_ID) REFERENCES LOCATION(LOCATION_ID);
ALTER TABLE EMPLOYEE ADD /*  REIGN_KEY_2  */
   FOREIGN KEY (JOB_ID) REFERENCES JOB(JOB_ID);
ALTER TABLE EMPLOYEE ADD /*  LF_KEY  */
   FOREIGN KEY (MANAGER_ID) REFERENCES EMPLOYEE(EMPLOYEE_ID);
ALTER TABLE EMPLOYEE ADD /*  REIGN_KEY_1  */
   FOREIGN KEY (DEPARTMENT_ID) REFERENCES DEPARTMENT(DEPARTMENT_ID);
ALTER TABLE ITEM ADD /*  RDER_FOREIGN_KEY  */
   FOREIGN KEY (ORDER_ID) REFERENCES SALES_ORDER(ORDER_ID);
ALTER TABLE ITEM ADD /*  RODUCT_FOREIGN_KEY  */
   FOREIGN KEY (PRODUCT_ID) REFERENCES PRODUCT(PRODUCT_ID);
ALTER TABLE PRICE ADD /*  FOREIGN_KEY  */
   FOREIGN KEY (PRODUCT_ID) REFERENCES PRODUCT(PRODUCT_ID);
ALTER TABLE SALES_ORDER ADD /*  ORDER_FOREIGN_KEY  */
